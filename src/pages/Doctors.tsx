...
const geocodeCity = async (city: string) => {
  const res = await fetch(
    `http://127.0.0.1:8000/api/geocode?city=${encodeURIComponent(city)}`
  );
...
const fetchDoctorsFromBackend = async (
...
): Promise<Doctor[]> => 
 {
  ...
  const res = await fetch(
    `http://127.0.0.1:8000/api/nearby-doctors?lat=${lat}&lon=${lon}&radius=${radiusKm * 1000}`
  );
  ...
  return data.map((doc: any) => {
    const dist =
      doc.lat && doc.lon
        ? calcDist(userLat, userLon, doc.lat, doc.lon)
        : 999;

    return {
      id: `osm-${doc.id}`,
      name: doc.name || "Women's Health Clinic",
      specialization: "Women's Health Specialist",
      hospital: doc.hospital || null,
      location: doc.address || null,
...
const Doctors = () => {
  ...
  const runNearbySearch = useCallback(async (lat: number, lon: number, limitKm: number, admins: Doctor[]) => {
    ...
    try {
      const overpass = await fetchDoctorsFromBackend(
  lat,
  lon,
  limitKm,
  lat,
